...
const paymentt_Checkout_top2_1 = document.querySelector(
  ".payment_Checkout_top2_1 "
);
const paymentt_Checkout_top2_2 = document.querySelector(
...
paybtnn2.addEventListener("click", () => {
...
let total1 = localStorage.getItem("sumTotal")??0;
let sum1 = localStorage.getItem("sumTotal");

for (let i = 0; i < cartArray?.length; i++) {
  const data = { _id: `${cartArray[i].id._id}` };
  fetch(" https://boat-backend-1ffa.onrender.com/api/products", {
    method: "POST", // or 'PUT'
    headers: {
...
<h5 id="quantityy_${datas[0]._id}">Quantity:  ${cartArray[i].item}</h5>
      ...
      cartmain.addEventListener("click", (e) => {
        ...
        let checkoutQuatityy = document.querySelector(
          `#quantityy_${datas[0]._id}`
        );
        ...
        let originalprice = document.querySelector(`#updated_${datas[0]._id}`);
